...
The IETF Deterministic Networking (DetNet) architecture~\cite{rfc8655} provides a complementary IP-layer framework for bounded latency and packet loss, combining traffic engineering, resource reservation, and path redundancy. Unlike TinyNet, DetNet requires per-flow forwarding state at each node and offline configuration of forwarding resources, placing it firmly in the stateful, controller-dependent category.
...
discovery---does not appear together in any of the protocols we surveyed,
